...
class AppHySpider(scrapy.Spider):
    name = 'app_hy'
    allowed_domains = ['hy.jxzbtb.cn']
    start_urls = ['http://hy.jxzbtb.cn/jygg/1.html']
    base_url = 'http://hy.jxzbtb.cn'
        ...
        for node in node_list:
            ...
            if page_now == 1 and nowItem == 0:
                ...
                send_email(receiver=['16396355@qq.com', '8206741@163.com'],
                           ...
                           title=self.curr_time + '嘉兴海盐县招标网站',
                           cont='<h1>今日爬取地址{}\r\n<br>嘉兴海盐县招标网站最新更新日期是{}</h1>'.format(response.url + "\r\n",
                                                                                     self.newday))
            nowItem += 1
    ...
    def newparse(self, response):
        ...
        item["noticeTitle"] = str(
            ''.join(response.xpath("//*[@id='jyxxChangeHeight']/h1/text()").extract()).encode(self.newEndcode),
            'utf-8').strip()

        yield item
if __name__ == "__main__":
    cmdline.execute("scrapy crawl app_hy".split())